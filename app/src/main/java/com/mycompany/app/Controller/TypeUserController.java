...
public class TypeUserController {
    ...
    public ResponseEntity<?> getAllTypeUsers() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al obtener los tipos de usuario: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> getTypeUserById(@PathVariable Long id) {
        try {
            ...
            if (typeUserDTO == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body("Tipo de usuario no encontrado con ID: " + id);
            }
            return ResponseEntity.ok(typeUserDTO);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al obtener el tipo de usuario: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> createTypeUser(@RequestBody TypeUserDTO typeUserDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Error al crear tipo de usuario: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> updateTypeUser(@PathVariable Long id,
        ...
        try {
            ...
            if (updatedTypeUser == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body("No se pudo actualizar - Tipo de usuario no encontrado con ID: " + id);
            }
            return ResponseEntity.ok("Tipo de usuario '" + updatedTypeUser.getType() + "' actualizado exitosamente");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Error al actualizar tipo de usuario: " + e.getMessage());
        }
    }
