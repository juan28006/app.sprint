...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    public MachineryDTO createMachinery(MachineryDTO machineryDTO) throws Exception {
        try {
            ...
            // 4. Validar ID de orden (obligatorio)
            if (machineryDTO.getOrder() == null || machineryDTO.getOrder().getId() == null) {
                throw new Exception("Se requiere un ID de orden válido");
            }

            // 5. Verificar estado del inventario
            ...
            // 6. Obtener la orden asociada
            OrderDTO order = orderDao.getOrderById(machineryDTO.getOrder().getId());
            if (order == null) {
                throw new Exception("Orden no encontrada con ID: " + machineryDTO.getOrder().getId());
            }

            // 7. Validar coincidencia de nombres (o asignar automáticamente)
            if (machineryDTO.getName() == null) {
                // Asignar nombre de la orden si no se proporciona
                machineryDTO.setName(order.getName());
            } else if (!machineryDTO.getName().equalsIgnoreCase(order.getName())) {
                throw new Exception(
                        String.format(
                                "El nombre de la maquinaria ('%s') debe coincidir con el de la orden asociada ('%s')",
                                machineryDTO.getName(), order.getName()));
            }

            // 8. Validar factura pagada (requerimiento adicional)
            ...
            if (!hasPaidInvoice) {
                throw new Exception("No se puede crear maquinaria. La orden no tiene facturas pagadas");
            }

            // 9. Establecer estado por defecto
            if (machineryDTO.getStatus() == null) {
                machineryDTO.setStatus("Disponible");
            }

            // 10. Delegar al DAO
            MachineryDTO createdMachinery = machineryDao.createMachinery(machineryDTO);

            // 11. Log de éxito
            System.out.printf(
                    "Maquinaria '%s' creada exitosamente. Orden: %s, Inventario: %s%n",
                    createdMachinery.getName(),
                    order.getId(),
                    inventory.getId());
            ...
            throw new Exception("Error de integridad en la BD: " + e.getMostSpecificCause().getMessage());
        } catch (Exception e) {
    ...
    public OrderDTO createOrder(OrderDTO orderDTO) throws Exception {
        ...
        if (inventory == null) {
            throw new Exception("Inventario no encontrado con ID: " + orderDTO.getInventory().getId());
        }

        // Validar que el inventario esté operativo
        if (!"Operativo".equalsIgnoreCase(inventory.getStatus())) {
            throw new Exception("No se puede crear la orden. El inventario asociado debe estar en estado 'Operativo'");
        }
