    ...
    public ReportInvoicesDTO updateReportFacturacion(ReportInvoicesDTO reportDTO) throws Exception {
        try {
            ...
            ReportInvoices existingReport = reportInvoicesRepository.findById(reportDTO.getId())
                    .orElseThrow(() -> new Exception("Factura no encontrada con ID: " + reportDTO.getId()));

            // 2. Manejar la orden (solo si se proporciona en el DTO)
            if (reportDTO.getOrder() != null && reportDTO.getOrder().getId() != null) {
                // Si la orden es la misma, no necesitamos hacer nada
                if (existingReport.getOrder() == null ||
                        !existingReport.getOrder().getId().equals(reportDTO.getOrder().getId())) {
            ...
            if (reportDTO.getEstado() != null) {
                ...
                if ("Pagada".equalsIgnoreCase(reportDTO.getEstado()) &&
                        !"Aprobada".equalsIgnoreCase(existingReport.getOrder().getStatus())) {
                    throw new Exception("No se puede marcar como pagada una factura de orden no aprobada");
                }
