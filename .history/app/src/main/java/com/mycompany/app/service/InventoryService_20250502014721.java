...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    private TypeUserDTO getOrCreateTypeUser(String type, String defaultPermissions) throws Exception {
        try {
            // 1. Buscar por nombre (ignorando ID)
            Optional<TypeUser> existingType = typeUserRepository.findByType(type);
    ...
    public UserDTO createUser(UserDTO userDTO) throws Exception {
        userValidator.validateForCreation(userDTO); // Usa el validador inyectado
    ...
    public UserDTO createAdmin(UserDTO userDTO) throws Exception {
        ...
        // Obtener o crear el tipo Admin (sin depender del ID)
        ...
        // 4. Crear nueva instancia para evitar problemas de referencia
        TypeUserDTO newTypeUser = new TypeUserDTO();
        newTypeUser.setType(adminType.getType());
        newTypeUser.setPermissions(adminType.getPermissions()); // Asegurar que los permisos no sean nulos
        ...
        // 6. Asignar permisos predeterminados (por si acaso)
        assignDefaultPermissions(createdUser);
    ...
    public UserDTO createEmpleado(UserDTO userDTO) throws Exception {
        ...
        newTypeUser.setPermissions(empleadoType.getPermissions()); // Asegurar que los permisos no sean nulos
        userDTO.setTypeUser(newTypeUser);
