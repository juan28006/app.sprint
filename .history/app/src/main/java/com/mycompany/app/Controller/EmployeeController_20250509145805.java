...
public class EmployeeController implements ControllerInterface {
    ...
    public ResponseEntity<?> createMachinery(@RequestBody MachineryDTO machineryDTO) {
        try {
            ...
            // Validación de facturas pendientes (modificada)
            List<ReportInvoicesDTO> facturasPendientes = reportService.getReportsByStatus("Pendiente").stream()
                    .filter(f -> {
                        try {
                            return f.getOrder() != null &&
                                    orderService.getOrderById(f.getOrder().getId()) != null && // ← Verifica si la orden
                                                                                               // existe
                                    f.getOrder().getMachinery() != null &&
                                    f.getOrder().getMachinery().getInventory() != null &&
                                    f.getOrder().getMachinery().getInventory().getId().equals(inventory.getId());
                        } catch (Exception e) {
                            return false; // Si la orden no existe, ignora esta factura
                        }
                    })
                    .collect(Collectors.toList());
            if (!facturasPendientes.isEmpty()) {
                return ResponseEntity.status(HttpStatus.PAYMENT_REQUIRED).body(Map.of(
                        "error", "Factura pendiente de pago",
                        "message",
                        "No se puede registrar la máquina porque hay facturas pendientes para este inventario",
                        "facturasPendientes", facturasPendientes.stream()
                                .map(f -> Map.of(
                                        "id", f.getId(),
                                        "codigoFactura", f.getCodigoFactura(),
                                        "total", f.getTotal()))
                                .collect(Collectors.toList()),
                        "timestamp", LocalDateTime.now()));
            }
