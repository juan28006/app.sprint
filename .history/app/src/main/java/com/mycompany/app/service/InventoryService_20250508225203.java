...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    @Override
    public ReportInvoicesDTO updateReportFacturacion(Long id, ReportInvoicesDTO reportDTO) {
        if (id == null || id <= 0) {
            throw new IllegalArgumentException("ID de factura inválido: " + id);
        }
        if (reportDTO == null) {
        ...
        try {
            ...
            return reportInvoicesDao.updateReportFacturacion(existingReportDTO);
        } catch (Exception e) {
