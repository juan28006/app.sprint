...
public class AdminController implements ControllerInterface {
    ...
    public ResponseEntity<?> updateReportInvoices(@PathVariable Long id, @RequestBody ReportInvoicesDTO reportDTO) {
        try {
            // Asegurar que el ID del body coincida con el de la URL si viene
            if (reportDTO.getId() != null && !reportDTO.getId().equals(id)) {
                return ResponseEntity.badRequest().body(Map.of(
                    "error", "Inconsistencia de IDs",
                    "message": "El ID en la URL no coincide con el ID en el cuerpo"
                ));
            }
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("error", "Error al actualizar factura", "message", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> deleteReportInvoices(@PathVariable Long id) {
        try {
            // 1. Validar que el ID no sea nulo o negativo
            if (id == null || id <= 0) {
                return ResponseEntity.badRequest().body(Map.of(
                        "error", "ID inválido",
                        "message", "El ID de la factura debe ser un número positivo",
                        "timestamp", LocalDateTime.now()));
            }
            ...
            // 3. Validar que la factura no esté asociada a una orden pagada
            if ("Pagada".equalsIgnoreCase(invoice.getEstado())) {
                return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of(
                        "error", "Factura pagada",
            ...
            // 4. Validar que no esté asociada a maquinaria en uso
            if (invoice.getOrder() != null && invoice.getOrder().getMachinery() != null) {
                String machineryStatus = invoice.getOrder().getMachinery().getStatus();
                if ("En uso".equalsIgnoreCase(machineryStatus)) {
                    return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of(
                            "error", "Maquinaria en uso",
                            "message", "No se puede eliminar la factura porque está asociada a maquinaria en uso",
                            "maquinariaId", invoice.getOrder().getMachinery().getId(),
                            "estadoMaquinaria", machineryStatus,
                            "timestamp", LocalDateTime.now()));
                }
            }
            ...
            return ResponseEntity.ok(Map.of(
                    ...
                    "facturaId", id,
                    "codigoFactura", invoice.getCodigoFactura(),
                    "timestamp", LocalDateTime.now()));
