    ...
    public MachineryDTO createMachinery(MachineryDTO machineryDTO) throws Exception {
        try {
            ...
            if (machineryDTO.getOrder() == null || machineryDTO.getOrder().getId() == null) {
                throw new Exception("Se requiere una orden válida con ID");
            }
            ...
            // Validar que la orden tiene un usuario creador
            if (order.getCreatedBy() == null || order.getCreatedBy().getId() == null) {
                throw new Exception("La orden no tiene un usuario creador válido");
            }
            ...
            if (!hasPaidInvoice) {
                throw new Exception("No se puede crear maquinaria. La orden con ID " + order.getId()
            ...
            // Conversión segura a entidad
            Machinery machinery = new Machinery();
            machinery.setName(machineryDTO.getName());
            machinery.setStatus(machineryDTO.getStatus());

            // Establecer relaciones con solo IDs
            Inventory inventoryEntity = new Inventory();
            inventoryEntity.setId(machineryDTO.getInventory().getId());
            machinery.setInventory(inventoryEntity);
            ...
            throw new Exception("Error al crear maquinaria: " + e.getMessage(), e);
        }
    }
