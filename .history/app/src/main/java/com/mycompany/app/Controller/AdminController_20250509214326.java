...
public class AdminController implements ControllerInterface {
    ...
    public ResponseEntity<?> createOrder(@RequestBody OrderDTO orderDTO) {
        try {
            // 1. Validación básica de objeto nulo
            if (orderDTO == null) {
                return ResponseEntity.badRequest().body(Map.of(
                        "error", "Datos requeridos faltantes",
            ...
            if (orderDTO.getCreatedBy() == null || orderDTO.getCreatedBy().getId() == null) {
                return ResponseEntity.badRequest().body("Debe especificar un usuario creador");
            }

            if (orderDTO.getInventory() == null || orderDTO.getInventory().getId() == null) {
                return ResponseEntity.badRequest().body("Debe especificar un inventario");
            }

            // 2. Generación automática de valores (si no vienen en el request)
            orderDTO.setOrderDate(new Date(System.currentTimeMillis())); // Fecha actual
            orderDTO.setOrderNumber("ORD-" + System.currentTimeMillis()); // Número único
            orderDTO.setStatus("Pendiente"); // Estado inicial

            // 3. Cálculo automático del total (si no se envió)
            ...
            // 4. Delegar la creación al Service (que a su vez llama al DAO)
            ...
            // 6. Manejo de errores (HTTP 400 Bad Request)
