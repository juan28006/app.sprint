...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    public void deletePerson(PersonDTO personDto) throws Exception {
        // Validaciones adicionales pueden ir aquí
        if (personDto == null) {
            throw new Exception("El DTO de persona no puede ser nulo");
        }

        // Verificar si la persona está asociada a algún usuario antes de eliminar
        UserDTO user = userDao.findByPersonDocument(personDto.getDocument());
        if (user != null) {
            throw new Exception(
                    "No se puede eliminar la persona porque está asociada al usuario: " + user.getUsername());
        }
