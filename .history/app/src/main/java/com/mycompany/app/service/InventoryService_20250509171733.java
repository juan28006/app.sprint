...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    public ReportInvoicesDTO updateReportFacturacion(Long id, ReportInvoicesDTO reportDTO) throws Exception {
        ...
        if (reportDTO == null) {
            throw new Exception("El DTO de la factura no puede ser nulo");
        }

        try {
            ReportInvoicesDTO existingReportDTO = reportInvoicesDao.getReportById(id);
            if (existingReportDTO == null) {
                throw new Exception("Reporte no encontrado con ID: " + id);
            }

            // Asignar el ID de la URL al DTO para asegurar consistencia
            reportDTO.setId(id);

            // Conditional updates
            if (reportDTO.getCodigoFactura() != null) {
                existingReportDTO.setCodigoFactura(reportDTO.getCodigoFactura());
            }
            if (reportDTO.getTotal() != null) {
                existingReportDTO.setTotal(reportDTO.getTotal());
            }
            if (reportDTO.getFechaGeneracion() != null) {
                existingReportDTO.setFechaGeneracion(reportDTO.getFechaGeneracion());
            }
            if (reportDTO.getEstado() != null) {
                existingReportDTO.setEstado(reportDTO.getEstado());
            }
            if (reportDTO.getOrder() != null) {
                existingReportDTO.setOrder(reportDTO.getOrder());
            }

            // Pasar ambos parámetros (id y DTO) al DAO
            return reportInvoicesDao.updateReportFacturacion(id, existingReportDTO);
        } catch (Exception e) {
            throw new Exception("Error al actualizar reporte de facturación: " + e.getMessage());
        }
    }
