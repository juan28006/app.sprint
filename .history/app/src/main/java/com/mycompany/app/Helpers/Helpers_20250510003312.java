...
public abstract class Helpers {
    ...
    // En el método parse(Machinery machinery)
    public static MachineryDTO parse(Machinery machinery) {
        ...
        dto.setInventory(parse(machinery.getInventory()));

        // Convertir Order a OrderDTO (versión simplificada)
        if (machinery.getOrder() != null) {
            OrderDTO orderDTO = new OrderDTO();
            orderDTO.setId(machinery.getOrder().getId());
            orderDTO.setName(machinery.getOrder().getName());
            // Agregar otros campos necesarios del OrderDTO
    ...
    // En el método parse(MachineryDTO dto)
    public static Machinery parse(MachineryDTO dto) {
        if (dto == null)
            return null;

        Machinery machinery = new Machinery();
        machinery.setId(dto.getId());
        ...
        // Order (solo ID) - Conversión segura
    ...
    public static OrderDTO parse(Order order) {
        ...
        // Convertir CreatedBy (solo ID y username)
        if (order.getCreatedBy() != null) {
            UserDTO userDTO = new UserDTO();
            userDTO.setId(order.getCreatedBy().getId());
            userDTO.setUsername(order.getCreatedBy().getUsername());
            orderDTO.setCreatedBy(userDTO);
        }

        // Convertir Inventory (solo ID)
        if (order.getInventory() != null) {
            InventoryDTO inventoryDTO = new InventoryDTO();
            inventoryDTO.setId(order.getInventory().getId());
            orderDTO.setInventory(inventoryDTO);
        }

        return orderDTO;
    }

    public static Order parse(OrderDTO orderDTO) throws Exception {
        if (orderDTO == null) {
            return null;
        }
        ...
        // Manejo seguro del usuario creador
        if (orderDTO.getCreatedBy() != null) {
            User user = new User();
            user.setId(orderDTO.getCreatedBy().getId()); // Solo necesitamos el ID
            order.setCreatedBy(user);
        } else {
            throw new Exception("El creador de la orden es obligatorio");
        }

        // Manejo seguro del inventario
        if (orderDTO.getInventory() != null) {
            Inventory inventory = new Inventory();
            inventory.setId(orderDTO.getInventory().getId()); // Solo necesitamos el ID
    ...
    // En Helpers.java
    // Método corregido en Helpers.java
    public static ReportInvoicesDTO parse(ReportInvoices report) {
        if (report == null) {
            return null;
        }
        ReportInvoicesDTO dto = new ReportInvoicesDTO();
        dto.setId(report.getId());
        dto.setCodigoFactura(report.getCodigoFactura());
        dto.setFechaGeneracion(report.getFechaGeneracion());
        dto.setTotal(report.getTotal());
        dto.setEstado(report.getEstado());

        // Mapeo simplificado de Order
        if (report.getOrder() != null) {
            OrderDTO orderDTO = new OrderDTO();
            orderDTO.setId(report.getOrder().getId());
            orderDTO.setOrderNumber(report.getOrder().getOrderNumber());
            orderDTO.setName(report.getOrder().getName()); // Nuevo campo
            dto.setOrder(orderDTO);
        }

        // Mapeo simplificado de User para evitar recursión
        if (report.getUsuario() != null) {
            UserDTO userDTO = new UserDTO();
            userDTO.setId(report.getUsuario().getId());
            userDTO.setUsername(report.getUsuario().getUsername());
    ...
    // Conversión de ReportInvoicesDTO a ReportInvoices (actualizado)
    public static ReportInvoices parse(ReportInvoicesDTO dto) {
        ...
        // Mapeo simplificado de User
        if (dto.getUsuario() != null) {
            User user = new User();
            user.setId(dto.getUsuario().getId());
            report.setUsuario(user);
        }
