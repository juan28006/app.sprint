...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    private TypeUserDTO getOrCreateTypeUser(String type, String defaultPermissions) throws Exception {
        try {
            ...
            // Crear nuevo tipo con permisos garantizados
            TypeUserDTO newType = new TypeUserDTO();
            newType.setType(type);
            newType.setPermissions(defaultPermissions); // Asignación segura

            // Validar
            typeUserValidator.validate(newType);

            // Guardar en BD
            TypeUserDTO savedType = typeUserDao.createTypeUser(newType);
            return savedType;
        } catch (Exception e) {
    ...
    public UserDTO createAdmin(UserDTO userDTO) throws Exception {
        userValidator.validate(userDTO); // Validación básica del usuario
        ...
        // Forzar la asignación de permisos aunque vengan nulos en el DTO
        if (userDTO.getTypeUser() == null) {
            userDTO.setTypeUser(new TypeUserDTO());
        }
        userDTO.getTypeUser().setPermissions(adminType.getPermissions());
        userDTO.getTypeUser().setType("Admin");
    ...
    public UserDTO createEmpleado(UserDTO userDTO) throws Exception {
        ...
        userDTO.getTypeUser().setPermissions(empleadoType.getPermissions());
        userDTO.getTypeUser().setType("Empleado");
    ...
    public UserDTO createCliente(UserDTO userDTO) throws Exception {
        ...
        userDTO.getTypeUser().setPermissions(clienteType.getPermissions());
        userDTO.getTypeUser().setType("cliente");
