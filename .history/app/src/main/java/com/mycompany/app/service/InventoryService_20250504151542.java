...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    public UserDTO createUser(UserDTO userDTO) throws Exception {
        ...
        if (userDTO.isAuthenticated() == false) {
    ...
    public UserDTO createAdmin(UserDTO userDTO) throws Exception {
        ...
        // Limpiar cualquier TypeUser existente en el DTO para evitar conflictos
        userDTO.setTypeUser(null);

        // Obtener o crear el tipo Admin con permisos específicos
        ...
        // Asignar el tipo de usuario con permisos al DTO
        /* userDTO.setTypeUser(adminType); */

        TypeUserDTO newTypeUser = new TypeUserDTO();
        newTypeUser.setType(adminType.getType());
        newTypeUser.setPermissions(adminType.getPermissions());
        userDTO.setTypeUser(newTypeUser);

        // 5. Crear y retornar usuario
        UserDTO createdUser = userDao.createUser(userDTO);
    ...
    public UserDTO createEmpleado(UserDTO userDTO) throws Exception {
        ...
        // 2. Limpiar TypeUser existente para evitar conflictos
        userDTO.setTypeUser(null);

        // 3. Obtener o crear tipo Empleado con permisos específicos
        TypeUserDTO empleadoType = getOrCreateTypeUser("Empleado",
                "Consultar_disponibilidad,Controlar_acceso,Iniciar_sesion,Registro");

        // 4. Configurar nuevo TypeUser con los permisos
        TypeUserDTO newTypeUser = new TypeUserDTO();
        newTypeUser.setType(empleadoType.getType());
        newTypeUser.setPermissions(empleadoType.getPermissions());
        userDTO.setTypeUser(newTypeUser);
    ...
    public UserDTO createCliente(UserDTO userDTO) throws Exception {
        ...
        // 2. Limpiar TypeUser existente
        userDTO.setTypeUser(null);

        // 3. Obtener o crear tipo Cliente
        TypeUserDTO clienteType = getOrCreateTypeUser("Cliente",
                "Consultar_disponibilidad,Iniciar_sesion,Registro,Reservar_maquinaria");

        // 4. Configurar nuevo TypeUser
        TypeUserDTO newTypeUser = new TypeUserDTO();
        newTypeUser.setType(clienteType.getType());
        newTypeUser.setPermissions(clienteType.getPermissions());
        userDTO.setTypeUser(newTypeUser);
    ...
    // Admin: Generar_informes
    public ReportDTO generateReport(ReportDTO reportDTO) throws Exception {
        permissionValidator.validatePermission(reportDTO.getUser(), "Generar_informes");
        // Lógica de generación de reportes
        return reportDAO.save(reportDTO);
    }

    // Admin: Gestionar_inventario
    public InventoryDTO manageInventory(InventoryDTO inventoryDTO) throws Exception {
        permissionValidator.validatePermission(inventoryDTO.getUser(), "Gestionar_inventario");
        // Lógica de gestión de inventario
        return inventoryDAO.update(inventoryDTO);
    }

    // Employee: Controlar_acceso
    public AccessLogDTO controlAccess(AccessRequestDTO accessDTO) throws Exception {
        permissionValidator.validatePermission(accessDTO.getUser(), "Controlar_acceso");
        // Lógica de control de acceso
        return accessService.logAccess(accessDTO);
    }

    // Client: Reservar_maquinaria
    public ReservationDTO createReservation(ReservationDTO reservationDTO) throws Exception {
        permissionValidator.validatePermission(reservationDTO.getUser(), "Reservar_maquinaria");
        // Lógica de reserva
        return reservationDAO.create(reservationDTO);
    }

    // Usa userDao en lugar de userRepository para mantener consistencia con DTOs
    UserDTO user = userDao.getUserById(userId);if(user==null)
    {
        throw new Exception("Usuario no encontrado");
    }

    // Verifica permisos (ajusta según la estructura de tus DTOs)
    String[] permissions = user.getTypeUser().getPermissions()
            .split(",");if(!Arrays.asList(permissions).contains("Reservar_maquinaria"))
    {
    ...
    public void assignDefaultPermissions(UserDTO userDTO) throws Exception {
        ...
        // Asignar permisos predeterminados si son nulos
        if (typeUserDTO.getPermissions() == null) {
            String permissions = getDefaultPermissionsForType(typeUserDTO.getType());
            typeUserDTO.setPermissions(permissions);
    ...
    public Object getAvailableMachinery() {
        // TODO Auto-generated method stub
        throw new UnsupportedOperationException("Unimplemented method 'getAvailableMachinery'");
    }
}