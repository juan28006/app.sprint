...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    public void deleteUser(Long id) throws Exception {
        ...
        try {
            ...
            // 2. Manejar registros de inventario relacionados
            List<Inventory> userInventories = inventoryRepository.findByUserId(id);
            for (Inventory inventory : userInventories) {
                inventory.setUser(null); // Desvincula el usuario del inventario
                inventoryRepository.save(inventory);
            }

            // 3. Manejar órdenes relacionadas
            List<Order> userOrders = orderRepository.findByCreatedById(id);
            for (Order order : userOrders) {
                // Opción 1: Eliminar las órdenes (si es permitido por negocio)
                orderRepository.delete(order);

                // Opción 2: Asignar a otro usuario o marcar como anónimo
                // order.setCreatedBy(adminUser);
                // orderRepository.save(order);
            }

            // 4. Manejar reportes de facturación relacionados
            List<ReportInvoices> userInvoices = reportInvoicesRepository.findByUsuarioId(id);
            for (ReportInvoices invoice : userInvoices) {
                // Similar a órdenes, decidir si eliminar o reasignar
                reportInvoicesRepository.delete(invoice);
            }

            // 5. Manejar reservaciones relacionadas
    ...
    public InventoryDTO createInventory(InventoryDTO inventoryDTO) throws Exception {
        ...
        if (inventoryDTO.getStatus() == null) {
            throw new Exception("El estado del inventario es obligatorio");
        }
    ...
    @Override
    public ReportInvoicesDTO generateReportInvoices(OrderDTO orderDTO) throws Exception {
        // Validación 1: Objeto no nulo
        if (orderDTO == null) {
            throw new Exception("El objeto OrderDTO no puede ser nulo");
        }
