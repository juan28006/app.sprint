...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    // In InventoryService implementation
    @Override
    public boolean hasUserDependencies(Long userId) throws Exception {
        // Check if user has any orders
        boolean hasOrders = !orderDao.getOrdersByUser(userId).isEmpty();

        // Check if user has any reports
        boolean hasReports = !reportInvoicesDao.getReportsByUsuarioId(userId).isEmpty();

        // Check if user has any reservations
        boolean hasReservations = !reservationDao.getReservationsByUser(userId).isEmpty();

        // Check if user has any inventory items
        boolean hasInventory = !inventoryDao.getInventoryByUser(userId).isEmpty();

        return hasOrders || hasReports || hasReservations || hasInventory;
    }
