...
public class TypeUserController {
    ...
    public ResponseEntity<?> deleteTypeUser(@PathVariable Long id) {
        try {
            boolean deleted = typeUserService.deleteTypeUser(id);
            if (!deleted) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body("No se encontró el tipo de usuario con ID " + id + " para eliminar");
            }
            return ResponseEntity.ok("Tipo de usuario con ID " + id + " eliminado correctamente");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Error al eliminar el tipo de usuario: " + e.getMessage());
        }
    }
}