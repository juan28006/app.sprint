...
public abstract class Helpers {
    ...
    public static OrderDTO parse(Order order) {
        ...
        orderDTO.setName(order.getName()); // Nuevo campo
        orderDTO.setStatus(order.getStatus());
        orderDTO.setCreatedBy(parse(order.getCreatedBy()));
        orderDTO.setInventory(parse(order.getInventory())); // Nuevo campo
        orderDTO.setQuantity(order.getQuantity());
        orderDTO.setUnitPrice(order.getUnitPrice());
        orderDTO.setTotalPrice(order.getTotalPrice());

        // Mapeo de facturas si existen
        if (order.getInvoices() != null) {
            List<ReportInvoicesDTO> invoiceDTOs = order.getInvoices().stream()
                    .map(Helpers::parse)
    ...
    public static Order parse(OrderDTO orderDTO) throws Exception {
        ...
        // Validación y mapeo seguro del usuario creador
        if (orderDTO.getCreatedBy() == null) {
            throw new Exception("El campo 'createdBy' es obligatorio");
        }
        ...
        User user = new User();
        user.setId(orderDTO.getCreatedBy().getId());
        order.setCreatedBy(user);

        // Validación y mapeo seguro del inventario
        if (orderDTO.getInventory() == null) {
            throw new Exception("El campo 'inventory' es obligatorio");
        }
        ...
        Inventory inventory = new Inventory();
        inventory.setId(orderDTO.getInventory().getId());
