...
public class AdminController implements ControllerInterface {
    ...
    public ResponseEntity<?> generateInvoices(@RequestBody OrderDTO orderDTO) {
        try {
            ...
            // 1. Generar código de factura más corto (ej: "FACT-240531-0154")
            SimpleDateFormat sdf = new SimpleDateFormat("yyMMdd-HHmm");
            String codigoFactura = "FACT-" + sdf.format(new Date());
            report.setCodigoFactura(codigoFactura);

            // 2. Fecha de generación (actual)
            report.setFechaGeneracion(new Date());

            // 3. Total de la orden
            report.setTotal(order.getTotalPrice());

            // 4. Asociar orden y usuario
            report.setOrder(order);
            report.setUsuario(order.getCreatedBy());

            // 5. Lógica para el estado (ej: "Pagado" si ya se pagó, sino "Pendiente")
            String estadoFactura = "Pendiente"; // Valor por defecto
            if (order.getPaymentStatus() != null && order.getPaymentStatus().equals("PAID")) {
                estadoFactura = "Pagado";
            } else if (order.getPaymentStatus() != null && order.getPaymentStatus().equals("CANCELLED")) {
                estadoFactura = "Anulado";
            }
            report.setEstado(estadoFactura);
