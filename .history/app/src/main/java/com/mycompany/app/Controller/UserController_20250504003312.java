...
public class UserController {
    ...
    public ResponseEntity<?> createAdmin(@RequestBody UserDTO userDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body("Administrador' " + createdAdmin.getUsername() + " 'creado exitosamente con ID:'"
                            + createdAdmin.getId() + "'" + "role " + ":" + "'Admin'");
        } catch (Exception e) {
    ...
    public ResponseEntity<?> createEmpleado(@RequestBody UserDTO userDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body("Empleado'" + createdEmployee.getUsername() + " 'creado exitosamente con ID:'"
                            + createdEmployee.getId() + "'" + "role" + ":" + "empleado");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of(
    ...
    public ResponseEntity<?> createCliente(@RequestBody UserDTO userDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body("Cliente'" + createdClient.getUsername() + " 'creado exitosamente con ID:'"
                            + createdClient.getId() + "'" + "role" + ":" + "Cliente");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of(
