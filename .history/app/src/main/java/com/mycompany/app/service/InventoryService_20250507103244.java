...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    public ReportDTO createReport(ReportDTO reportDTO) throws Exception {
        // Validación 1: Objeto no nulo
        if (reportDTO == null) {
            throw new Exception("El objeto ReportDTO no puede ser nulo");
        }
        ...
        // Obtener el usuario de la base de datos
        UserDTO user = userDao.getUserById(reportDTO.getUser().getId());

        // Validación 3: Usuario existe
        if (user == null) {
            throw new Exception(
                    "No se encontró usuario con ID: " + reportDTO.getUser().getId() + ". Registre al usuario primero.");
        }

        // Validación 4: Usuario autenticado
        if (!user.isAuthenticated()) {
            throw new Exception(
                    "El usuario con ID: " + user.getId() + " no está autenticado. No puede generar reportes.");
        }

        // Validación 5: Permisos del usuario
        if (!hasPermission(user, "Generar_informes")) {
            throw new Exception("El usuario no tiene permisos para generar reportes");
        }

        // Validación 6: Campos obligatorios del reporte
        if (reportDTO.getType() == null || reportDTO.getType().trim().isEmpty()) {
            throw new Exception("El tipo de reporte es obligatorio");
        }

        // Si pasa todas las validaciones, crear el reporte
        try {
            ReportDTO createdReport = reportDao.createReport(reportDTO);

            System.out.println("Reporte generado exitosamente por el usuario autenticado: " +
                    user.getUsername() + " (ID: " + user.getId() + ")");
    ...
    @Override
    public ReportInvoicesDTO getInvoiceById(Long id) throws Exception {
        try {
            ReportInvoicesDTO invoice = reportInvoicesDao.getInvoiceById(id);
            if (invoice == null) {
