...
public class ReservationImplementation implements ReservationDao {
    ...
    @Autowired
    private MachineRepository machineryRepository;
    ...
    public ReservationDTO createReservation(ReservationDTO reservationDTO) throws Exception {
        try {
            ...
            if (reservationDTO == null) {
                throw new Exception("Los datos de reserva no pueden ser nulos");
            }

            // 2. Validar maquinaria
            if (reservationDTO.getMachinery() == null || reservationDTO.getMachinery().getId() == null) {
                throw new Exception("Se requiere ID de maquinaria");
            }

            // Obtener maquinaria directamente del repositorio para evitar joins complejos
            Machinery machinery = machineryRepository.findById(reservationDTO.getMachinery().getId())
                    .orElseThrow(() -> new Exception(
                            "Maquinaria no encontrada con ID: " + reservationDTO.getMachinery().getId()));

            // Verificar estado de la maquinaria
            if (!"Disponible".equalsIgnoreCase(machinery.getStatus())) {
                throw new Exception("La maquinaria con ID " + machinery.getId() + " no está disponible. Estado actual: "
                        + machinery.getStatus());
            }

            // 3. Validar usuario
            if (reservationDTO.getUser() == null || reservationDTO.getUser().getId() == null) {
                throw new Exception("Se requiere ID de usuario");
            }

            // 4. Establecer valores por defecto
            if (reservationDTO.getReservationDate() == null) {
                reservationDTO.setReservationDate(new Date()); // Fecha y hora actual
            }

            if (reservationDTO.getStatus() == null || reservationDTO.getStatus().isEmpty()) {
                reservationDTO.setStatus("Pendiente");
            }

            // 5. Verificar conflictos de fecha
            ...
            if (existsConflict) {
                throw new Exception("La maquinaria ya está reservada para la fecha y hora seleccionada");
            }
            ...
            // Establecer relaciones solo con IDs para evitar problemas
            User user = new User();
            user.setId(reservationDTO.getUser().getId());
            reservation.setUser(user);

            reservation.setMachinery(machinery);

            Reservation savedReservation = reservationRepository.save(reservation);

            // 7. Retornar DTO con los datos básicos
            ReservationDTO result = new ReservationDTO();
            result.setId(savedReservation.getId());
            result.setReservationDate(savedReservation.getReservationDate());
            result.setStatus(savedReservation.getStatus());

            // Establecer solo IDs en los DTOs relacionados
            UserDTO userDTO = new UserDTO();
            userDTO.setId(savedReservation.getUser().getId());
            result.setUser(userDTO);

            MachineryDTO machineryDTO = new MachineryDTO();
            machineryDTO.setId(savedReservation.getMachinery().getId());
            result.setMachinery(machineryDTO);
