...
import com.mycompany.app.dto.AccessLogDTO;
...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    public AccessLogDTO controlAccess(AccessRequestDTO accessDTO) throws Exception {
        validatePermission(accessDTO.getUser(), "Controlar_acceso");

        AccessLogDTO log = new AccessLogDTO();
        log.setUser(accessDTO.getUser());
        log.setAccessTime(new Date(0));
        log.setArea(accessDTO.getArea());
        log.setReason(accessDTO.getMotivo());
    ...
    // ==================== EXECUTE WITH PERMISSION WRAPPER ====================
    public <T> ResponseEntity<T> executeWithPermission(UserDTO user, String permission, Supplier<T> action)
            throws Exception {
        try {
