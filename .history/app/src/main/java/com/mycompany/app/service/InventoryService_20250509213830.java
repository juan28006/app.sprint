...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    public MachineryDTO createMachinery(MachineryDTO machineryDTO) throws Exception {
        ...
        // Validación 2: Nombre de máquina obligatorio y coincidente con órdenes
        ...
        // Validación 6: Buscar facturas pagadas asociadas a esas órdenes
        boolean hasPaidInvoice = false;
        for (OrderDTO order : ordersWithSameName) {
            List<ReportInvoicesDTO> invoices = reportInvoicesDao.getReportsByOrderId(order.getId());
            for (ReportInvoicesDTO invoice : invoices) {
                if ("Pagada".equalsIgnoreCase(invoice.getEstado())) {
                    hasPaidInvoice = true;
                    break;
                }
            }
        ...
        if (!hasPaidInvoice) {
            throw new Exception("No se puede crear maquinaria sin una factura pagada asociada a una orden con nombre: "
                    + machineryDTO.getName());
        }
