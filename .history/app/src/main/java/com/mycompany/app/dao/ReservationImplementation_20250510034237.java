...
public class ReservationImplementation implements ReservationDao {
    ...
    public ReservationDTO createReservation(ReservationDTO reservationDTO) throws Exception {
        try {
            // 1. Validate reservationDTO is not null
            if (reservationDTO == null) {
                throw new Exception("Reservation data cannot be null");
            }

            // 2. Validate machinery exists and is available
            ...
            MachineryDTO machinery = machineryDao.getMachineryById(reservationDTO.getMachinery().getId());
            if (machinery == null) {
                throw new Exception("Maquinaria no encontrada con ID: " + reservationDTO.getMachinery().getId());
            }

            // Check machinery status
            if (!"Disponible".equalsIgnoreCase(machinery.getStatus())) {
                throw new Exception("Machinery with ID " + machinery.getId() + " No está disponible. Estado actual: "
                        + machinery.getStatus());
            }
            ...
            // 4. Set automatic fields
            // Set current date as reservation date if not provided
            if (reservationDTO.getReservationDate() == null) {
                reservationDTO.setReservationDate(new Date()); // Current date and time
            }

            // Set status to "Pendiente" if not provided
            ...
            if (existsConflict) {
                throw new Exception("Maquinaria ya reservada para la fecha y hora seleccionada");
            }
