...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService, ReportService,
    ...
    public UserDTO createUser(UserDTO userDTO) throws Exception {
        ...
        try {
            ...
            createdUser.setAuthenticated(true); // Esto ahora debería funcionar
            return createdUser;
        } catch (Exception e) {
    ...
    public UserDTO createAdmin(UserDTO userDTO) throws Exception {
        ...
        // Buscar el tipo "Admin" en la base de datos
        TypeUserDTO adminType = typeUserDao.getAllTypeUsers().stream()
                .filter(t -> "Admin".equalsIgnoreCase(t.getType()))
                .findFirst()
                .orElseThrow(() -> new Exception("El tipo 'Admin' no existe. Crea primero el tipo de usuario."));

        // Asignar el tipo encontrado al usuario
        userDTO.setTypeUser(adminType);

        // Crear el usuario (los permisos ya están en el TypeUser)
