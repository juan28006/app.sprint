...
public class TypeUserImplementation implements TypeUserDao {
    ...
    @Override
    public TypeUserDTO findByType(String type) throws Exception {
        try {
            return typeUserRepository.findByType(type)
                    .map(Helpers::parse)
                    .orElse(null);
        } catch (Exception e) {
            throw new Exception("Error al buscar tipo de usuario por nombre", e);
        }
    }
    ...
    public TypeUserDTO createTypeUser(TypeUserDTO typeUserDTO) throws Exception {
        try {
            ...
            return Helpers.parse(savedTypeUser);
        } catch (Exception e) {
