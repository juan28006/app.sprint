...
public class InventoryService implements UserService, TypeUserService, InventoryS, ReservationService,
    ...
    @Autowired
    private MachineryRepository machineRepository;
    ...
    public MachineryDTO createMachinery(MachineryDTO machineryDTO) throws Exception {
        ...
        OrderDTO orders = orderDao.getOrderById(machineryDTO.getOrder().getId());
        if (orders == null) {
            throw new Exception("No se encontró orden con ID: " + machineryDTO.getOrder().getId());
        }

        // Tomar el nombre de la orden si no se especificó
        if (machineryDTO.getName() == null) {
            machineryDTO.setName(orders.getName());
        }

        // Validación 2: Nombre de máquina obligatorio
        ...
        // Validación 3: Inventario asociado
        ...
        // Validación 4: Estado del inventario
        ...
        // Validación 5: Buscar órdenes con el mismo nombre de máquina
        List<OrderDTO> ordersWithSameName = orderDao.getOrdersByNameMaquina(machineryDTO.getName());
        if (ordersWithSameName.isEmpty()) {
            throw new Exception("No existe ninguna orden con el nombre de máquina: " + machineryDTO.getName());
        }

        // Validación 6: Verificar si hay más de una orden con el mismo nombre
        if (ordersWithSameName.size() > 1) {
            throw new Exception("Existen múltiples órdenes con el nombre de máquina: " + machineryDTO.getName()
                    + ". Por favor, especifique el ID de la orden correcta.");
        }

        // Validación 7: Buscar facturas pagadas asociadas a la orden
        OrderDTO order = ordersWithSameName.get(0); // Tomamos la primera (y única) orden
        List<ReportInvoicesDTO> invoices = reportInvoicesDao.getReportsByOrderId(order.getId());

        boolean hasPaidInvoice = invoices.stream()
                .anyMatch(invoice -> "Pagada".equalsIgnoreCase(invoice.getEstado()));

        if (!hasPaidInvoice) {
            throw new Exception("No se puede crear maquinaria. La orden con nombre '" + machineryDTO.getName()
                    + "' no tiene facturas pagadas.");
        }
        // Al guardar, se mantiene solo la referencia al ID
    ...
    public OrderDTO createOrder(OrderDTO orderDTO) throws Exception {
        ...
        // Validación 3: Usuario asociado existe y está autenticado
        ...
        try {
            ...
            if (reportDTO.getEstado() != null) {
                if ("Pagada".equalsIgnoreCase(reportDTO.getEstado())) {
                    // Validar que la orden asociada existe
                    ...
                    // Obtener la orden actual (no la del DTO)
            ...
            return reportInvoicesDao.updateReportFacturacion(existingInvoice);
        } catch (Exception e) {
