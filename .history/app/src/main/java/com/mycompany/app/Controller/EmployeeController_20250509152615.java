...
public class EmployeeController implements ControllerInterface {
    ...
    public ResponseEntity<?> createMachinery(@RequestBody MachineryDTO machineryDTO) {
        try {
            ...
            // Nueva validación: Verificar si existe orden con factura pagada para este
            // inventario
            List<OrderDTO> orders = orderService.getOrdersByInventoryId(inventory.getId());

            boolean hasPaidInvoice = orders.stream()
                    .anyMatch(order -> {
                        try {
                            List<ReportInvoicesDTO> invoices = reportService.getReportsByOrderId(order.getId());
            ...
            // Si pasa todas las validaciones, crear la maquinaria
